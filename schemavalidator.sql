...
SELECT src.schema_name "schema Name", src.size_in_gb "Oracle Size(GB)", COALESCE(tgt.size_in_gb ,0)"PostgreSQL Size(GB)"
  FROM (select owner schema_name, round(sum(bytes::bigint)/1024/1024/1024,0) size_in_gb from dba_segments where owner=upper(:'ora_schema') group by owner) src
       LEFT OUTER JOIN
       (select schemaname schema_name, round(sum(pg_total_relation_size(schemaname||'.'||tablename))/1024/1024/1024,0) size_in_gb from pg_tables where schemaname=:'pg_schema' group by schemaname) tgt
...
select src.owner "Schema Name", src.table_name "Table Name", src.index_name "Index Name" ,  src.index_type "Oracle Index Type"
  from (select i.owner, i.table_name, i.index_name, i.generated, i.index_type, STRING_AGG(coalesce(tc.column_id,0)::text, ' ' order by ic.column_position) indkey
	   ...
	   ON lower(src.table_name)=tgt.table_name and src.indkey = tgt.indkey
...
WHERE c.relname = table_name
 ...
 postgresql.data_type  as "PostgreSQL Data Type" FROM ORACLE inner join postgresql
 on owner = table_schema and ORACLE.table_name = postgresql.table_name
...
select upper(pa.table_schema) "Parent Schema", upper(pa.table_name) "Parent Table Name", upper(pa.column_name) "Parent Column name", format_type(paa.atttypid, paa.atttypmod) "PostgreSQL Parent Data type",
 ...
 where ch.table_schema=lower(ora_tbl.chc_owner) and ch.table_name=lower(ora_tbl.chc_table_name) and ch.column_name=lower(ora_tbl.chc_column_name)
   and pa.table_schema=lower(ora_tbl.pac_owner) and pa.table_name=lower(ora_tbl.pac_table_name) and pa.column_name=lower(ora_tbl.pac_column_name)
